...
namespace TichdiemDrGreen.Areas.Admin.Controllers
{
    ...
    public class CustomerController : Controller
    {
        ...
        public ActionResult Index(string searchstring, int? page, string from_date, string to_date, string sex, string from_year, string to_year, string callby, string from_calldate)
        {
            var model = from a in db.Customers
                        select a;
            if (!string.IsNullOrEmpty(searchstring))
            {
                searchstring = MyControl.formatUserId(searchstring, 0);
